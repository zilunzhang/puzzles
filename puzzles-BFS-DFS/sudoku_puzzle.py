...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return true if the SudokuPuzzle is unsolvable.
        ...
        for i in range(len(self._symbols)):
            # find the allowed symbols for *
            if self._symbols[i] == "*":
                allowed_symbols = (self._symbol_set - (
                    self._row_set(i) | self._column_set(i) |
                    self._subsquare_set(i)))
                # if allowed_symbols for i is an empty set, return true
